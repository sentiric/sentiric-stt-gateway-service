...
impl AppConfig {
    pub fn load() -> Result<Self> {
        let builder = Config::builder()
            ...
            .set_override_option(
                "stt_whisper_service_grpc_url",
                env::var("STT_WHISPER_SERVICE_GRPC_URL").ok(),
            )?
            // DEFAULTS
            ...
            .set_default(
                "stt_whisper_service_grpc_url",
                "https://stt-whisper-service:15031",
            )?
            .set_default("grpc_tls_ca_path", "/sentiric-certificates/certs/ca.crt")?
            .set_default(
                "stt_gateway_service_cert_path",
                "/sentiric-certificates/certs/stt-gateway-service.crt",
            )?
            .set_default(
                "stt_gateway_service_key_path",
                "/sentiric-certificates/certs/stt-gateway-service.key",
            )?;
